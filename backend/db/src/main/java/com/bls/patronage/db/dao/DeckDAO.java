...
public abstract class DeckDAO {
    ...
    @SqlQuery("select decks.id, decks.name, decks.public, decks.authorEmail from decks join usersDecks on usersDecks.deckId = decks.id " +
    ...
    @SqlUpdate("insert into decks (id, name, public, authorEmail) values (:id, :name, :isPublic, :authorEmail)")
    ...
    @SqlUpdate("update decks set name = :name, public = :isPublic, authorEmail = :authorEmail where id = :id")
    ...
    @SqlQuery("select id, name, public, authorEmail from decks where id = :id")
    abstract Deck getDeck(@Bind("id") UUID id);

    @SqlQuery("select id, name, public, authorEmail from decks where name like :name and public='true'")
    abstract List<Deck> getDecksUsingName(@Bind("name") String name);

    @SqlQuery("select id, name, public, authorEmail from decks " +
    ...
    @SqlQuery("select id, name, public, authorEmail from decks where public='true'")
